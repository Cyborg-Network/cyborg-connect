...
import { generateX25519KeyPair, X25519KeyPair } from "../../util/non-bc-crypto/generateX25519KeyPair";
...
export const useAgentCommunication = (metadata: any) => {
  ...
  const [diffieHellmanSecret, setDiffieHellmanSecret] = useState<Uint8Array | null>(null);
  ...
  const [lockState, setLockState] = useState<LockState>({isLocked: true, isLoading: false});
