...
const handleTransactionEvents = (api: ApiPromise, events: any) => {
...
export const useTransaction = (api: ApiPromise) => {
  ...
  const handleTransaction = async ({ tx, account, onSuccess, onError }) => {
    if (!api || !tx || !account) {
      console.error(
        'Missing required parameters: API, transaction, or account.'
      )
      return
    }
    ...
    try {
      await tx.signAndSend(...fromAcct, ({ status, events, dispatchError }) => {
        ...
        if (status.isInBlock || status.isFinalized) {
          ...
          const { hasErrored, successfulEvents, errorEvents } =
            handleTransactionEvents(api, events)
